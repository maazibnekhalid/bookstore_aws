...
resource "aws_lb_target_group" "order_service" {
  ...
  tags = { Name = "${local.name}-order-tg" }
}

# HTTP listener: redirects to HTTPS if a cert is provided, otherwise serves the
# Product Service directly on port 80 (dev/no-TLS mode).
